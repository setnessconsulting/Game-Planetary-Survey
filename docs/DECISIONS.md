...
## D-41 — Production art is original and generated by our own pipeline, and the
provenance manifest is generated with it

**Decision:** The v1 production art is **original procedural work** produced by
`scripts/generate-production-assets.mjs`, with `agency origin: none` and no
third-party or comparator material. The same script emits
`src/assets/assetManifest.json` and `src/assets/provenanceManifest.json` from the
loop that wrote the bytes. No placeholder ships.

**Rationale:** `docs/ASSET_PROVENANCE.md` makes unknown provenance release-blocking
and states that using agency *data* as a scientific source is not a licence for
agency *imagery*. PS-05 satisfied the letter of that with generated placeholders and
deferred the decision; PS-10 had to make it. Owner direction (2026-09-25) chose
original procedural art over agency-derived textures and over externally licensed
art. That choice removes the licence question entirely: there is nothing to
attribute, nothing to clear, and nothing that could be mistaken for an agency
product. It also means the art is reproducible from source, which is what makes the
recorded content hashes evidence rather than decoration.

Generating the manifests from the same loop that writes the files removes a whole
class of drift. The previous manifest was hand-written and the asset gate recovered
entries with regular expressions over TypeScript source; a template literal or a
factory function registered nothing and the gate stayed green.

**Consequence:** The art is original and machine-generated, **not
artist-commissioned**. Human visual sign-off is therefore still outstanding and
belongs to PS-14; `docs/ART_DIRECTION.md` exists to tell that reviewer what they are
looking at. `scripts/check-asset-pipeline.mjs` now fails the build on any
placeholder, on a dangling `provenanceId`, on an incomplete or unapproved record, on
a record with a non-`none` agency origin, and on a byte count or content hash that
disagrees with the file on disk.

## D-42 — Shipping textures are PNG, because the approved dependency set cannot
decode KTX2

**Decision:** Shipping textures are PNG, hand-encoded with Node's built-in `zlib`.
The KTX2/Basis preference in `docs/RENDERING_QUALITY_STRATEGY.md` §8 is **not**
met, and that deviation is recorded here rather than taken silently.

**Rationale:** §8 says the choice per texture is "recorded, not guessed", which is
exactly this situation. `@babylonjs/loaders@9.28.0` contains no KTX2 decoder at all
— no `KTX2Loader`, no `KTXTexture`; the Basis transcoder exists in
`@babylonjs/core/assets/Basis` but is not wired to the KTX2 container. The PS-05
manifest shipped a 260-byte KTX2 albedo that therefore could never be decoded, and
because the loader swallowed the failure and fell back to a flat PBR colour, the
qualified guided-mission slice had been rendering **no texture whatsoever** for five
stories. Shipping a texture the browser cannot read is worse than shipping a larger
one it can, so PNG is the honest answer within the current dependency set.

**Consequence:** Textures are roughly 3–5× larger than a Basis-compressed
equivalent would be. At ~450–560 KB per body the set is still far inside the
per-body (4 MiB) and total (60 MiB) budgets in
`docs/PERFORMANCE_AND_DEVICE_BUDGETS.md` §3.3, and the bundle report confirms they
stay in the lazy payload. Reverting to KTX2 requires a dependency change, which
D-20 treats as a separately reviewed change that re-runs the whole gate — it is
deliberately not smuggled in here.

## D-43 — Audio starts muted, and every cue declares a non-audio equivalent

**Decision:** The audio service defaults to **muted** (`DEFAULT_MUTED = true`), and
every cue in `src/audio/cues.ts` declares a `nonAudioEquivalent`.

**Rationale:** `docs/ACCESSIBILITY.md` A-10 requires audio to be off by default and
to carry nothing required. The shell previously started *unmuted*, which was
harmless only because `play()` was a no-op. Making the cues real without flipping
the default would have shipped newly audible sound to a school device — the change
that turns a harmless default into a defect. Declaring the equivalent per cue is
what makes A-10 checkable rather than aspirational: an essential cue with no
equivalent is a test failure, not a reviewer's memory.

**Consequence:** Cues are synthesised from oscillators and noise, so no audio file
ships, no decoder or loader is needed, and no runtime dependency was added. Each cue
still carries a provenance record, because `docs/ASSET_PROVENANCE.md` requires audio
to be present in the manifest even when it has no bytes. `src/audio/cues.ts` imports
nothing from `src/domain` or `src/ui`, which is the structural form of "audio must
not advance mission state", and a test fails the build if that changes.

## D-44 — The renderer reads the motion tokens, and the atmosphere shell is a
labelled representation rather than a derived one

**Decision:** Two presentation decisions that the code previously contradicted.

1. The camera transition duration comes from `--ps-motion-camera` through
   `cameraTransitionMs()` in `src/design/motion.ts`, not from a literal in the
   renderer.
2. The atmosphere shell is a **neutral, explicitly labelled representation** with a
   fixed presentation scale, not a shell sized from the sourced attribute.

**Rationale:** (1) `src/renderer/cameraModes.ts` carried
`CAMERA_TRANSITION_MS = 900` while the token said `640ms`, so the product had two
camera-step durations and editing the token would have moved the DOM and left the
camera where it was. `docs/DESIGN_SYSTEM.md` §6 requires the camera step to be
included in the reduced-motion override because "a rule the renderer can opt out of
is not a rule", and a renderer holding its own duration is precisely that.

(2) §4 of `docs/RENDERING_QUALITY_STRATEGY.md` requires an atmospheric shell whose
depth is derived from a sourced attribute, **and** forbids rendering a
plausible-looking shell that implies a measurement the game does not have. In v1
exactly one body has a sourced atmosphere value, and it is a **lower bound on the
altitude at which the atmosphere was detected**, not a measured top. Scaling a shell
from it would draw a precise boundary the register does not contain. §4 provides the
alternative: a neutral, explicitly labelled representation plus learner text.

**Consequence:** Shell visibility is still gated on
`bodyAvailableAttributes.includes("atmosphereDepth")`, so only a world with a
sourced atmosphere shows one, and its presence asserts presence and nothing about
depth. Deriving a real depth is left open for PS-11/PS-12, when a register entry
carrying an actual measured depth would let the shell be honest. The camera's
authored 640 ms also sits inside the ≤1200 ms camera budget, so following the token
costs nothing.

## D-45 — Production art loads on snapshot change, and the slice gate proves it

**Decision:** The renderer loads a body's production art whenever the projected
snapshot's `bodyId` changes, not once at start-up; the canvas publishes
`data-ps-art-body` and `data-ps-art-notes`; and the `@slice` real-browser test
asserts both for every world the guided mission surveys.

**Rationale:** Producing production art and then actually looking at the screen
found three defects that five stories of green tests had not:

1. **The glTF 2.0 loader was never registered.** `src/renderer/assets.ts`
   imported `@babylonjs/loaders/glTF/glTFFileLoader`, which registers only the
   *file* plugin. The glTF 2.0 factory is installed by a separate
   `RegisterGLTF2Loader()` call, so `GLTFFileLoader._CreateGLTF2Loader` was
   `undefined` and **every `.glb` was rejected with "Unsupported version: 2.0"** —
   an error that reads like a corrupt file but means a missing factory. The fix
   is importing `@babylonjs/loaders/glTF/2.0/glTFLoader`.
2. **Nothing triggered the load.** The body was loaded once at controller
   start-up, when no mission exists and `bodyId` is still `null`. Selecting a
   world afterwards never re-triggered it, so the scene sat on its untextured
   fallback sphere for the whole session.
3. **A texture-readiness race.** `new Texture(url)` can finish decoding before
   `onLoadObservable` is subscribed, so the await could sit until timeout and
   report a loaded texture as failed.

None of these was visible because **the viewport looks identical either way**.
A renderer showing a world and a renderer showing a bare sphere are the same
size, the same colour, and the same frame counter. The guided-mission slice
completed, the screenshots had a planet in them, and the console was clean,
because the loop never depended on the art.

That is the real lesson, and it is why this decision is about the *gate* as much
as the loader: the observable (`data-ps-art-body`) and the assertion are the
deliverable. A story that ships assets without a way to see that the assets
loaded has not shipped assets.

**Consequence:** The fallback sphere is now only reachable when art genuinely
fails, it is reported through `data-ps-art-notes` and the degraded status
region, and switching bodies disposes the previous mesh instead of leaking one
GLB per world visited. The texture helper holds the materials it created rather
than re-discovering them with `instanceof`, which would silently do nothing if
the bundle ever held two copies of the Babylon module.

## D-46 — The PS-05 placeholder generator is deleted, because its output is
now release-blocking

**Decision:** `scripts/generate-placeholder-assets.mjs` is removed.
`scripts/generate-production-assets.mjs` is the only supported way to produce
shipping art, and the hardened `check:assets` gate fails on any placeholder id,
path, or provenance id.

**Rationale:** The script was not merely superseded, it had become a footgun.
It writes `public/assets/bodies/placeholder-body.glb`,
`placeholder-albedo.ktx2`, and `placeholder-body-lod1.glb`, while
`check:assets` walks the whole repository looking for shipping extensions. One
`node scripts/generate-placeholder-assets.mjs` therefore breaks
`npm run verify` on two counts at once: the three new files are unregistered
shipping assets with no provenance record, and their paths match the
`/placeholder/i` check. A tool whose correct use is "do not run it" is not a
tool.

Nothing of value is lost by removing it. PS-05's evidence is a set of measured
byte counts and hashes recorded in the manifests of that commit, not a promise
that the bytes remain reproducible on demand, and the placeholder history stays
visible in D-34, in this entry, and in
`docs/ACCEPTANCE_EVIDENCE_MATRIX.md`.

**Consequence:** `generate-production-assets.mjs` honours `PS_ASSET_OUT_ROOT`
so tests can regenerate into a temporary directory instead of the working tree,
and the art-direction rules it encodes — contrast floors and the relief ceiling
from `docs/RENDERING_QUALITY_STRATEGY.md` §10 — are enforced at generation time
